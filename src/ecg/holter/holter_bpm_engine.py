...
class HolterBPMDisplay(QFrame):
    ...
    def _build_ui(self):
        ...
        self._elapsed_lbl.setStyleSheet(f"color: {COL_GREEN}; font-size: 18px; font-weight: bold; font-family: monospace;")
        layout.addWidget(self._elapsed_lbl)

        # Target (e.g. / 24:00:00)
        if self._target_hours > 0:
            tgt = QLabel(f"/ {self._target_hours}:00:00")
            tgt.setStyleSheet(f"color: {COL_GREEN_DRK}; font-size: 13px;")
        ...
        # Arrhythmia ticker
        self._ev_lbl = QLabel("")
        self._ev_lbl.setStyleSheet(f"color: {COL_GREEN}; font-size: 12px; font-weight: bold;")
        self._ev_lbl.setMaximumWidth(300)
    ...
    def _refresh_bpm(self):
        ...
        if arrhythmias:
            ...
            self._ev_lbl.setStyleSheet(
                f"color: {COL_GREEN}; font-size: 10px; font-weight: bold;")
