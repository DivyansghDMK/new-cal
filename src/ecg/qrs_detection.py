...
def get_authoritative_qrs_duration(lead_ii_data: np.ndarray, r_peaks: np.ndarray, fs: float) -> float:
    """
    Get a single authoritative QRS duration.
    Attempts to build a clinical median beat from Lead II. If successful, measures QRS duration on it.
    Otherwise, falls back to the median of raw per-beat QRS measurements.
    """
    from .clinical_measurements import build_median_beat, get_tp_baseline

    if len(lead_ii_data) == 0 or len(r_peaks) == 0:
        return 0.0
    ...
    if len(r_peaks) >= 2:
        ...
        rr_ms = np.median(valid_rr) if len(valid_rr) > 0 else 800.0
    else:
        rr_ms = 800.0
    estimated_bpm = 60000.0 / rr_ms if rr_ms > 0 else 75.0

    # Set min_beats dynamically based on heart rate
    if estimated_bpm < 40:
        min_beats = 1
    ...
    time_axis, median_beat = build_median_beat(lead_ii_data, r_peaks, fs, min_beats=min_beats)
    if median_beat is not None and time_axis is not None:
        try:
            # Find a central R peak to calculate TP baseline
            central_peak_idx = r_peaks[len(r_peaks) // 2]
            tp_baseline = get_tp_baseline(lead_ii_data, central_peak_idx, fs)
        except Exception:
            tp_baseline = None

        if tp_baseline is None:
            # Fallback baseline calculation on the median beat itself
            bl_samples = int(0.05 * fs)
            tp_baseline = float(np.mean(median_beat[:bl_samples])) if len(median_beat) > bl_samples else 0.0

        qrs_duration = measure_qrs_duration_paper(
            median_beat,
            time_axis,
            fs,
        ...
        if qrs_duration > 0:
            return float(qrs_duration)

    # Fallback: calculate QRS duration from raw signal around R peaks
    raw_durations = []
    for r_curr in r_peaks:
        try:
            dur = qrs_duration_from_raw_signal(
                lead_ii_data,
                int(r_curr),
                fs,
                adc_per_mv=1200.0,
                heart_rate=int(estimated_bpm)
            )
            if dur > 0:
    ...
    if raw_durations:
        return float(np.median(raw_durations))
    return 0.0
