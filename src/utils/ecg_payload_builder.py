...
def _load_signup_details(signup_details: Optional[Dict], patient: Dict, data: Dict) -> Dict:
    ...
    try:
        ...
        for _, rec in users.items():
            ...
            rec_login = str(
                rec.get("username")
                or rec.get("login_username")
                or rec.get("login_identifier")
                or rec.get("canonical_username")
                or ""
            ).strip()
            for key in candidate_keys:
                ...
                if rec_name and key.lower() == rec_name:
                    return dict(rec)
                if rec_login and key == rec_login:
                    return dict(rec)
        return {}
    except Exception:
        return {}
...
def _resolve_master_phone(patient: Dict, data: Dict, signup: Dict) -> str:
    """Resolve the canonical phone number for the logged-in owner."""
    candidates = [
        signup.get("master_phone"),
        signup.get("phone"),
        signup.get("contact"),
        patient.get("master_phone"),
        patient.get("phone"),
        patient.get("mobile_no"),
        data.get("master_phone"),
        data.get("phone"),
        data.get("mobile_no"),
    ]
...
def _build_patient_block(patient: Dict, data: Dict, report_id: str, signup: Optional[Dict] = None) -> Dict:
    ...
    master_phone = _resolve_master_phone(patient, data, signup or {})
...
def save_payload_locally(payload: Dict, reports_dir: str = "reports") -> Optional[str]:
    ...
    try:
        ...
        serial = (
            ...
            or payload.get("device_details", {}).get("machine_serial", "UNKNOWN")
        )
        report_type = payload.get("report_type", "unknown")
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
